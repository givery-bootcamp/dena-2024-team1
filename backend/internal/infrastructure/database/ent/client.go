...
import (
	...
	"myapp/internal/infrastructure/database/ent/migrate"
...
type Client struct {
	...
	Post *PostClient
	// Sketch is the client for interacting with the Sketch builders.
	Sketch *SketchClient
...
func (c *Client) init() {
	...
	c.Sketch = NewSketchClient(c.config)
...
func (c *Client) Intercept(interceptors ...Interceptor) {
	...
	c.Sketch.Intercept(interceptors...)
...
func (c *HelloWorldClient) UpdateOne(hw *HelloWorld) *HelloWorldUpdateOne {
	mutation := newHelloWorldMutation(c.config, OpUpdateOne, withHelloWorld(hw))
	return &HelloWorldUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *HelloWorldClient) UpdateOneID(id int) *HelloWorldUpdateOne {
...
func (c *HelloWorldClient) DeleteOne(hw *HelloWorld) *HelloWorldDeleteOne {
	return c.DeleteOneID(hw.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *HelloWorldClient) DeleteOneID(id int) *HelloWorldDeleteOne {
...
func (c *HelloWorldClient) Get(ctx context.Context, id int) (*HelloWorld, error) {
	return c.Query().Where(helloworld.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *HelloWorldClient) GetX(ctx context.Context, id int) *HelloWorld {
...
// SketchClient is a client for the Sketch schema.
type SketchClient struct {
	config
}

// NewSketchClient returns a client for the Sketch from the given config.
func NewSketchClient(c config) *SketchClient {
	return &SketchClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `sketch.Hooks(f(g(h())))`.
func (c *SketchClient) Use(hooks ...Hook) {
	c.hooks.Sketch = append(c.hooks.Sketch, hooks...)
}

// Intercept adds a list of query interceptors to the interceptors stack.
// A call to `Intercept(f, g, h)` equals to `sketch.Intercept(f(g(h())))`.
func (c *SketchClient) Intercept(interceptors ...Interceptor) {
	c.inters.Sketch = append(c.inters.Sketch, interceptors...)
}

// Create returns a builder for creating a Sketch entity.
func (c *SketchClient) Create() *SketchCreate {
	mutation := newSketchMutation(c.config, OpCreate)
	return &SketchCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// CreateBulk returns a builder for creating a bulk of Sketch entities.
func (c *SketchClient) CreateBulk(builders ...*SketchCreate) *SketchCreateBulk {
	return &SketchCreateBulk{config: c.config, builders: builders}
}

// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *SketchClient) MapCreateBulk(slice any, setFunc func(*SketchCreate, int)) *SketchCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &SketchCreateBulk{err: fmt.Errorf("calling to SketchClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*SketchCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
		setFunc(builders[i], i)
	}
...
// Update returns an update builder for Sketch.
func (c *SketchClient) Update() *SketchUpdate {
	mutation := newSketchMutation(c.config, OpUpdate)
	return &SketchUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOne returns an update builder for the given entity.
func (c *SketchClient) UpdateOne(s *Sketch) *SketchUpdateOne {
	mutation := newSketchMutation(c.config, OpUpdateOne, withSketch(s))
	return &SketchUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *SketchClient) UpdateOneID(id int) *SketchUpdateOne {
	mutation := newSketchMutation(c.config, OpUpdateOne, withSketchID(id))
	return &SketchUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for Sketch.
func (c *SketchClient) Delete() *SketchDelete {
	mutation := newSketchMutation(c.config, OpDelete)
	return &SketchDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// DeleteOne returns a builder for deleting the given entity.
func (c *SketchClient) DeleteOne(s *Sketch) *SketchDeleteOne {
	return c.DeleteOneID(s.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *SketchClient) DeleteOneID(id int) *SketchDeleteOne {
	builder := c.Delete().Where(sketch.ID(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
	return &SketchDeleteOne{builder}
}

// Query returns a query builder for Sketch.
func (c *SketchClient) Query() *SketchQuery {
	return &SketchQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeSketch},
		inters: c.Interceptors(),
	}
}

// Get returns a Sketch entity by its id.
func (c *SketchClient) Get(ctx context.Context, id int) (*Sketch, error) {
	return c.Query().Where(sketch.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *SketchClient) GetX(ctx context.Context, id int) *Sketch {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// QueryUser queries the user edge of a Sketch.
func (c *SketchClient) QueryUser(s *Sketch) *UserQuery {
	query := (&UserClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := s.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(sketch.Table, sketch.FieldID, id),
			sqlgraph.To(user.Table, user.FieldID),
...
// Hooks returns the client hooks.
func (c *SketchClient) Hooks() []Hook {
	return c.hooks.Sketch
}

// Interceptors returns the client interceptors.
func (c *SketchClient) Interceptors() []Interceptor {
	return c.inters.Sketch
}

func (c *SketchClient) mutate(ctx context.Context, m *SketchMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&SketchCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&SketchUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&SketchUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&SketchDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown Sketch mutation op: %q", m.Op())
	}
}
...
// QuerySketches queries the sketches edge of a User.
func (c *UserClient) QuerySketches(u *User) *SketchQuery {
	query := (&SketchClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := u.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(user.Table, user.FieldID, id),
			sqlgraph.To(sketch.Table, sketch.FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, user.SketchesTable, user.SketchesColumn),
		)
		fromV = sqlgraph.Neighbors(u.driver.Dialect(), step)
		return fromV, nil
	}
...
type (
	hooks struct {
		HelloWorld, Post, Sketch, User []ent.Hook
	}
	inters struct {
		HelloWorld, Post, Sketch, User []ent.Interceptor
	}
)
