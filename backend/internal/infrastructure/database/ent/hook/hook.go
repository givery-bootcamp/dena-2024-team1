...
func (f HelloWorldFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.HelloWorldMutation); ok {
		return f(ctx, mv)
	}
...
// The SketchFunc type is an adapter to allow the use of ordinary
// function as Sketch mutator.
type SketchFunc func(context.Context, *ent.SketchMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f SketchFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.SketchMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.SketchMutation", m)
}
